...
```bash
uv run hepagent run --agent "scientist" "how many python files in this code repository"
uv run hepagent run --agent "scientist" "I would like to simulate a cosmology sky with Nyx code." --max-turn 30
uv run hepagent run --agent "scientist" "..." --model "openai:gpt-5-mini"
uv run hepagent run --agent "scientist" "..." --model "gemini-flash"  # defaults to cborg
```
