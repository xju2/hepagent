...
* When it comes to creating SLURM scripts, always optimize for computing performance and resource usage. Always add `#SBATCH --mail-type=BEGIN,END,FAIL`. Ask the user for their email to include in the SLURM script. Ask for account and partition if needed. Always ask for user confirmation or suggestions before submitting the SLURM job.
