...
4. Use `PrintFormat=5` in the `input.par` if the grid size is smaller than 64^3 to avoid issues with illegal memory access when running `nyx` with GPUs.
5. After that, write a bash script that users can run the `init` in SLURM. Remember to put all output files into a subdirectory named `ic_output` to keep the working directory clean.
