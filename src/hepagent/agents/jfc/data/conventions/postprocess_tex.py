...
def fix_longtable_short(lines):
    ...
    def convert_if_short(match):
        ...
        data_lines = [
            letter
            for letter in body.split("\n")
            if "\\\\" in letter
            and "\\toprule" not in letter
            and "\\midrule" not in letter
            and "\\bottomrule" not in letter
            and "\\endhead" not in letter
            and "\\endfoot" not in letter
            and "\\endlastfoot" not in letter
        ]
        ...
        caption_match = re.search(r"\\caption\{.*?\}(?:\\label\{[^}]*\})?\\\\", new_body, re.DOTALL)
        caption = ""
        if caption_match:
    ...
    for i, line in enumerate(lines):
        ...
        if re.search(r"\\caption\{", line) or re.search(r"\\toprule|\\midrule", line):
            if re.search(r"\(4[abc]\)|Phase\s+[1-5]\b", line):
                warnings.append(f"  line {i + 1}: internal phase label in: {line.strip()[:80]}")
    if warnings:
        sys.stderr.write(
            f"WARNING: {len(warnings)} internal phase label(s) found in captions/headers:\n"
        )
        for w in warnings:
            sys.stderr.write(w + "\n")
...
def fix_margins(lines):
    ...
    for i, line in enumerate(lines):
        if re.search(r"\\usepackage\[.*\]\{geometry\}", line) or re.search(
            r"\\usepackage\{geometry\}", line
        ):
            if line == target:
...
def fix_abstract(lines):
    ...
    for i, line in enumerate(lines):
        ...
        if re.search(r"\\section\{Abstract\}", line) or re.search(r"\\section\[Abstract\]", line):
            abstract_start = i
...
def fix_figure_placement(lines):
    ...
    for i, line in enumerate(lines):
        ...
        if "\\pandocbounded{\\includegraphics[keepaspectratio" in line and "height=" not in line:
...
def fix_references_placement(lines):
    ...
    while insert_pos < len(lines) and lines[insert_pos].strip() in ("\\clearpage", ""):
...
def fix_appendix(lines):
    ...
    for i, line in enumerate(lines):
        if re.search(r"%%\s*Appendices", line) or re.search(r"<!--\s*Appendices\s*-->", line):
...
def fix_clearpage(lines):
    ...
    for _, idx in indices:
        ...
        while (
            insert_at > 0
            and lines[insert_at - 1].strip() in ("\\FloatBarrier", "")
